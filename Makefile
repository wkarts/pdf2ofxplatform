...
prod-install:
	@test -n "$(DOMAIN)" || (echo "Uso: make prod-install DOMAIN=https://dominio [VERSION=1.1.7]" >&2; exit 1)
	bash deploy/docker/install.sh --domain "$(DOMAIN)" --version "$(or $(VERSION),1.1.7)"
